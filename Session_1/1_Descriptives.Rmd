...
* A simple way to analyze whether specific information is available in a text is the key-words in context analysis. 
...
## Context: Political Speech and Divisivess
* Now let's have a look at the context:
* In module 4 we will measure the divisiveness of political speech.
* For that purpose, we will analyse Plenarprotokolle from the German Bundestag. 
* The formal explanation on how to measure divisiveness will be given in module 4
* For now, we will investigate the data.
...
I downloaded the .xml files and transformed them to using a Python script to .txt (`xml_to_txt.py`) beforehand. Then we add all .txt documents in the `data/prtkll/19WP` to our corpus.
...
### Characteristics
- Uses a combination of syllables and sentence length to indicate complexity of speech
...
```{r rdblty_plt,echo=FALSE, out.width='.49\\linewidth', fig.width=3, fig.height=3,fig.show='hold',fig.align='center', cache = TRUE}
...
## Document Frequency Matrix and Cosine Similarity

### Document Frequency Matrix
* A document-feature matrix (dfm) shows the documents in the rows, the features in the columns.
* Can be interpreted as the implementation of the bag-of-words concept (more about that later).
...
* The matrix shows that the second document is very similar to the reference document.
...
# Dictionary, summary statistics and key words in context
