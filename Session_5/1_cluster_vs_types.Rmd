...
```{r distsance, echo=FALSE, fig.align="center", cache=TRUE, fig.width=12}
include_graphics("figures/distance_measures.png")
```
...
```{r sample, echo=FALSE, fig.align="center", cache=TRUE, fig.width=12}
include_graphics("figures/dtw.png")
```
Figure from @Sarda-Espinosa2017

- The figure shows sample alignment performed by DTW between two series. 
- The dashed blue lines exemplify how some points are mapped to each other.
- In other words: It shows how they can be warped in time.
...
```{r opt_path, echo=FALSE, fig.align="center", cache=TRUE, fig.width=8}
include_graphics("figures/dtw_alignment.png")
```
...
- $\gamma$
  - If you are interested in the impact of $\gamma$, read @Cuturi2017, Table 1.
- window 
  - global constraint 
  - It is common to use a window whose size is 10% of the series’ length. 
  - Sometimes smaller windows produce even better results @Ratanamahatana2004a.
- norm
...
```{r proto, echo=FALSE, fig.align="center", cache=TRUE}
include_graphics("figures/summary_prototypes.png")
```
...
```{r hier, echo=FALSE, fig.align="center", cache=TRUE}
include_graphics("figures/hierarchical_clustering.png")
```

- As the level in the hierarchy increases, clusters are created by merging the clusters from the next lower level
- In order to decide how the merging is performed, a (dis)similarity measure between groups is specified.
- The procedure is deterministic, so it will always give the same result for a chosen set of (dis)similarity measures. 
...
```{r part, echo=FALSE, fig.align="center", cache=TRUE}
include_graphics("figures/partitional_clustering.png")
```
...
```{r fuzzy, echo=FALSE, fig.align="center", cache=TRUE}
include_graphics("figures/fuzzy_clustering.png")
```
...
#### Overview of Internal Cluster Validity Indices
```{r cvis_plot_theory, echo=FALSE, fig.align="center", cache=TRUE}
include_graphics("figures/internalCVIs.png")
```
...
```{r clustering, include=TRUE, cache=TRUE}
require(doParallel)
...
```{r cvis_plot, echo=FALSE, cache=TRUE, fig.align="center", fig.height=8}
